...
public class Insert_Student extends JFrame implements ActionListener
{
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        else
        {
            try
            {
                ...
                ResultSet r2;
                String sql2="SELECT Sno, Cno, Grade FROM SC WHERE Sno = \"" + s1 +"\" AND Cno = \"" + s2 + "\" AND Grade >= 0";

                Connect.con.setAutoCommit(false);
                r2=st.executeQuery(sql2);
                if(r2.next())
                {
                    JOptionPane.showMessageDialog(this, "成绩存在，不需要添加！");
                }
                else
                {
                    String sql = "INSERT INTO SC(Sno, Cno, Grade) VALUES (\'" + s1 + "\', \'"+ s2 + "\', " + s3 + " );";
    ...
    //public static void main(String[] args) { new Insert_Student(); }
}
